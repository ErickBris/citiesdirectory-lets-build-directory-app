...
class Cities extends REST_Controller
{
	...
	{
		$cats = $this->category->get_all($city_id)->result();
		foreach ($cats as $cat) {
	...
	function get_sub_categories($city_id, $cat_id)
	{
		...
		foreach ($sub_cats as $sub_cat) {
			//$sub_cat->items = $this->get_items($city_id, $sub_cat->id);
	...
	function feeds_get()
	{
		...
		$feeds = array();
		if ($count && $from) {
			$feeds = $this->feed->get_all($city_id, $count, $from)->result();
		} else if ($count) {
			$feeds = $this->feed->get_all($city_id, $count)->result();
		} else {
			$feeds = $this->feed->get_all($city_id)->result();
		}
