...
class Reviews extends Main
{
	...
	function index()
	{
		...
		$pag['total_rows'] = $this->review->count_all($this->get_current_city()->id);
		$data['reviews'] = $this->review->get_all($this->get_current_city()->id, $pag['per_page'], $this->uri->segment(3));
		$data['pag'] = $pag;
		$content['content'] = $this->load->view('reviews/list',$data,true);		
	...
	function search()
	{
		...
		$pag['total_rows'] = $this->review->count_all_by($this->get_current_city()->id, array('searchterm'=>$search_term));
